module Wechat::WcpayHelper
	...
	module PayXml
		...
		def self.get_xml(params,is_qrcode=false)
		  ...
		  if is_qrcode
			'weixin://wxpay/bizpayurl?'+params.collect{|k,v| "#{k}=#{v}&"}.join+"sign="+WcpayHelper::Sign.generate(params)
		  else
			"<xml>#{params.collect { |k, v| "<#{k}>#{v}</#{k}>" }.join}<sign>#{WcpayHelper::Sign.generate(params)}</sign></xml>"
		  end
		end
		...
		def self.get_json(params)
		   ...
		   params.merge({paySign: WcpayHelper::Sign.generate(params)})
		end
	end
	...
	module Redbage
		def self.get_xml(params)
			...
			client_ip: '123.59.61.178',
		     }.merge(params)
		     "<xml>#{params.collect{|key,value| "<#{key}>#{value}</#{key}>"}.join}<sign>#{WcpayHelper::Sign.generate(params)}</sign></xml>"
		end
	end
end

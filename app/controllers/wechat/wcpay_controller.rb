class Wechat::WcpayController < ApplicationController
	...
	def qr_pay
		...
		body=PayXml.get_xml(hash,true)
		 Qrcode.qcode(body)
		 qrcode=File.open(Rails.root.join("public","abc.png"))
		result=JSON.parse(Sangna.upload_media(auth_code.token,qrcode,'image','my_qrcode'))
		puts result
	end
	...
	def redbage
		...
		uri = URI(url)
                ...
                #  request.body=body
         p12 = OpenSSL::PKCS12.new(File.open(Rails.root.join("apiclient_cert.p12"),"rb").read,"1245225302")
		...
		response=RestClient::Resource.new(
 			 ...
 			 :ssl_ca_file      =>  Rails.root.join("rootca.pem") , 
